...
def oru(input: torch.Tensor, *, radix:float=1.) -> torch.Tensor:
  i_radix = 1/radix
  s_radix = radix ** radix

  return torch.mul(
    torch.sign(input),
    torch.pow((torch.mul(torch.abs(input), s_radix) + 1), i_radix) - 1)
...
class ORU(nn.Module):
  '''
  Applies the Odd Root Unit Unit (ORU) function element-wise.
  Args:
    radix: is a positive shape parameter that defines the choice of non-linearity
  Shape:
    - Input: (N, *) where * means, any number of additional
  ...
  def forward(self, input: torch.Tensor) -> torch.Tensor:
    return oru(input, radix=self.radix)